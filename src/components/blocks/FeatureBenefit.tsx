...
import { ACCENT, BlockFallback, HOVER_CARD, IconBubble, tokensFor, VisualShell, type Surface } from './_shared';
  ...
  return (
    <VisualShell surface={surface} Icon={Target} eyebrow={data.eyebrow || 'Feature → Benefit'} title={data.title || 'Why It Matters'}>
      <ul className="space-y-2.5">
        {pairs.map((pair, i) => (
          <li key={i} className={cn('group rounded-xl p-3.5', t.card, HOVER_CARD)}>
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              ...
              <ArrowRight
                className="hidden sm:block w-4 h-4 flex-shrink-0 rotate-90 sm:rotate-0 transition-transform duration-200 group-hover:translate-x-0.5 motion-reduce:transition-none"
                style={{ color: ACCENT }}
                aria-hidden="true"
              />
              <span className={cn('text-[14px] lg:text-[15px] leading-relaxed flex-1 transition-colors duration-200 group-hover:text-[#3A3338]', t.textSecondary)}>{pair.benefit}</span>
            </div>
          </li>
        ))}
      </ul>
