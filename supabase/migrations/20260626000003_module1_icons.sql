/*
  Module 1 — replace all raw emoji with consistent semantic icon names.

  Re-sets Module 1 to the same block array as 20260626000002 but every `icon`
  field now holds a stable semantic NAME (cloud, shield-check, rocket, …) that
  the block renderers resolve to a single lucide icon system. No more random
  emoji — iconography is uniform across every visual and badge.
