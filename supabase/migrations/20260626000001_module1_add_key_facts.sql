/*
  Module 1 — add a Key Facts strip to the "Who We Are" topic.

  Re-sets Module 1 video_url to the same block array as 20260625000002 with one
  new `key_facts` block (m1-facts) inserted right after the growth timeline.
  buildModule() groups consecutive visuals after a text section into one topic,
  so the facts strip renders below the timeline on topic 1.1.

  Facts are canonical (drawn from existing Module 1 content): 22,000+ customers,
  global presence, end-to-end infrastructure & cloud. No invented data.
*/
